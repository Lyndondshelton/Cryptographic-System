...
alpha_cypher = {' ': 0, 'A': 3, 'B': 6, 'C': 9, 'D': 12, 'E': 15, 'F': 18, 'G': 21, 'H': 24, 'I': 27, 'J': 30, 'K': 33,
                'L': 36, 'M': 39, 'N': 42, 'O': 45, 'P': 48, 'Q': 51, 'R': 54, 'S': 57, 'T': 60, 'U': 63, 'V': 66,
                'W': 69, 'X': 72, 'Y': 75, 'Z': 78, ',': 81, '.': 84, '?': 87, '!': 90, "'": 93, "-": 96}
alpha_decrypt = {0: ' ', 3: 'A', 6: 'B', 9: 'C', 12: 'D', 15: 'E', 18: 'F', 21: 'G', 24: 'H', 27: 'I', 30: 'J', 33: 'K',
                 ...
                 69: 'W', 72: 'X', 75: 'Y', 78: 'Z', 81: ',', 84: '.', 87: '?', 90: '!', 93: "'", 96: "-"}
...
INV_MATRIX_KEY = [
    [17/192, -103/192, 71/64, -113/192],
    [-25/192, 95/192, -63/64, 121/192],
    [5/192, -19/192, 51/64, -101/192],
    [29/192, 5/192, -37/64, 67/192]
]
...
dict_size = alpha_cypher["-"] + CAESAR_KEY
...
def encryption(s):
    ...
    #
    for x in alpha_cypher:
        alpha_cypher[x] = (alpha_cypher[x] + CAESAR_KEY) % dict_size
    ...
    for i in range(y_rows):
        ...
        for j in range(y_cols):
            Y[i].append(alpha_cypher[s[char_count]])
            char_count += 1
...
def decryption(enc_mat):
    ...
    for i in range(len(DEC)):
        for j in range(len(DEC[0])):
            DEC[i][j] = (DEC[i][j] - CAESAR_KEY) % dict_size
